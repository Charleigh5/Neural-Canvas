...
import { Terminal, Activity, ShieldAlert, Copy, Check } from 'lucide-react';
...
export const CouncilLogs: React.FC = () => {
  ...
  const handleCopy = () => {
    const text = logs
      .map(l => `[${new Date(l.time).toLocaleTimeString()}] ${l.type.toUpperCase()}: ${l.msg}`)
      .join('\n');
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar font-mono text-[9px]">
              {logs.length === 0 && (
                <div className="text-slate-600 italic">Standby... No transactions detected.</div>
              )}
              {logs.map((log, i) => (
