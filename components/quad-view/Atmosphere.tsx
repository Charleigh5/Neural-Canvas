...
export const AuroraBorealis = ({ theme }: { theme: BezelTheme }) => {
    ...
    return (
        <div className="absolute inset-0 z-0 opacity-40 pointer-events-none mix-blend-screen"
             style={{
                 background: 'linear-gradient(135deg, #020617 0%, #064e3b 50%, #1e1b4b 100%)',
...
export const BlizzardProtocol = ({ theme, density, config }: { theme: BezelTheme, density: number, config?: ThemeConfig }) => {
...
export const DigitalFrost = ({ active }: { active: boolean }) => {
    ...
    return (
        <div 
            className="absolute inset-0 pointer-events-none z-[60] mix-blend-screen transition-opacity duration-1000"
            style={{ opacity: frost }}
        >
            {/* Edge Frost */}
    ...
    return (
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-64 h-32 z-[5] pointer-events-none overflow-hidden flex justify-center">
             {Array.from({ length: 6 }).map((_, i) => (
                 <div 
                      key={i}
                      className="w-10 h-10 bg-white/10 rounded-full blur-2xl absolute bottom-0 animate-smoke"
                      style={{ 
...
export const FallingSnow = ({ theme, density, config }: { theme: BezelTheme, density: number, config?: ThemeConfig }) => {
