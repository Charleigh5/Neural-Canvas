...
const SequencerImage = ({ url, isCurrent }: { url: string, isCurrent: boolean }) => {
    ...
    if (status === 'failed' || !img) return <div className="w-full h-full bg-rose-900/20 flex items-center justify-center text-rose-500 text-xs font-mono">ERROR</div>;

    if (img instanceof ImageBitmap) {
        return (
            <canvas 
                ref={canvasRef}
                className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity pl-6"
                style={{ filter: isCurrent ? 'none' : 'grayscale(20%)' }}
            />
        );
    }
...
const MenuButton = ({ icon: Icon, label, onClick, color = "text-slate-300", hoverColor = "hover:bg-indigo-500/20" }: any) => (
    <button onClick={(e) => { e.stopPropagation(); onClick(); }} className={`flex items-center gap-3 w-full px-3 py-2 rounded-lg text-left transition-colors ${hoverColor} group`}>
        <Icon size={12} className={`${color} group-hover:scale-110 transition-transform`} />
        <span className={`text-[9px] font-bold uppercase tracking-wider ${color} group-hover:text-white`}>{label}</span>
    </button>
);
...
export const SequencerItem = React.memo(({ 
    ...
    return (
        ...
        >
            <div className={`relative w-48 h-32 rounded-xl overflow-hidden border-2 transition-all ${isVisible ? (isCurrent ? 'border-indigo-500 shadow-[0_0_20px_rgba(99,102,241,0.3)]' : 'bg-slate-900 border-white/10 group-hover:border-white/30') : 'bg-white/5 border-white/5'}`}>
                {isVisible ? (
                    <>
                        {/* Drag Handle */}
                        <div 
                        ...
                        {img.transition && (
                            <div className="absolute top-2 left-8 bg-black/40 rounded px-1.5 py-0.5 border border-white/10 backdrop-blur-md">
                                <span className="text-[8px] font-black uppercase text-indigo-300 tracking-widest">{img.transition}</span>
                            </div>
                        )}
                        ...
                        <div className="absolute bottom-2 left-8 right-2 flex justify-between items-end">
                            <div className="flex flex-col overflow-hidden">
                                <span className={`text-[9px] font-black uppercase leading-none mb-1 ${isCurrent ? 'text-indigo-300' : 'text-slate-500'}`}>SLIDE_{idx + 1}</span>
                                <span className="text-[8px] font-mono text-slate-400 truncate tracking-tight">{img.tags[0] || 'ANALYZING...'}</span>
                            </div>
                ...
                {isMenuOpen && isVisible && (
                    <>
                        ...
                        >
                            ...
                            <MenuButton icon={Maximize2} label="Upscale 4K" onClick={() => { onMenuAction(id, 'upscale'); setIsMenuOpen(false); }} />
                            <MenuButton icon={Crop} label="Smart Crop" onClick={() => { onMenuAction(id, 'crop'); setIsMenuOpen(false); }} />
                            <div className="h-px bg-white/5 my-1" />
                            <MenuButton icon={Eraser} label="Remove BG" onClick={() => { onMenuAction(id, 'remove_bg'); setIsMenuOpen(false); }} color="text-rose-400" hoverColor="hover:bg-rose-900/30" />
                        </motion.div>
                    </>
                )}
            </AnimatePresence>
