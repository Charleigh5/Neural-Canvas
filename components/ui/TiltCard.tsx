...
export const TiltCard: React.FC<HoloPanelProps> = ({ 
  ...
  return (
    <div
      onClick={onClick}
      className={`
        relative group overflow-hidden bg-slate-900/40 border transition-all duration-300 backdrop-blur-md
        ${selected 
    ...
    >
      ...
      <div className={`absolute top-0 left-0 w-3 h-3 border-t-2 border-l-2 transition-colors duration-300 ${selected ? 'border-indigo-400' : 'border-white/20 group-hover:border-indigo-500/50'}`} />
      <div className={`absolute bottom-0 right-0 w-3 h-3 border-b-2 border-r-2 transition-colors duration-300 ${selected ? 'border-indigo-400' : 'border-white/20 group-hover:border-indigo-500/50'}`} />
